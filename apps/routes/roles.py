...
# ==========================================================
# PROTECCIÓN EXCLUSIVA DEL MÓDULO ROLES Y PERMISOS
# ==========================================================

def acceso_roles_requerido(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # SUPERADMINISTRADOR
        # --------------------------------------------------
        # El Superadministrador NO tiene acceso a este módulo.
        # Este módulo pertenece exclusivamente a cada empresa.
        # --------------------------------------------------
        ...
        if not current_user.rol_objeto:

            abort(403)


        # --------------------------------------------------
        # SOLO ADMINISTRADOR DE EMPRESA
        # --------------------------------------------------

        nombre_rol = (
            current_user.rol_objeto.nombre
            .strip()
            .lower()
        )
        ...
        if not current_user.empresa.activa:

            abort(403)


        # --------------------------------------------------
        # EL ROL ADMINISTRADOR DEBE ESTAR ACTIVO
        # --------------------------------------------------

        if not current_user.rol_objeto.activo:
...
def aplicar_permisos_predeterminados(
    rol,
    modulos
):
